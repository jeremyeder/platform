...
import { MessageSquare, Loader2, Settings, Terminal, Users, ArrowUp, Square } from "lucide-react";
  ...
  return (
    <div className="flex flex-col h-full">
      <div
        ...
        className="flex-1 flex flex-col gap-2 overflow-y-auto p-3 scrollbar-hover"
      >
            ...
            <div className="mb-4 mt-2">
              <div className="flex space-x-3 items-start">
                ...
                <div className="flex-shrink-0">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center bg-muted-foreground/20">
                    <span className="text-foreground text-xs font-semibold">AI</span>
                  </div>
                </div>
      ...
      {!isInteractive && filteredMessages.length > 0 && (
        <div className="sticky bottom-0 bg-surface-primary-alt border-t border-border-light">
          <div className="p-3">
      ...
      {showChatInterface && (
        <div className="border-t bg-surface-primary">
          <div className="max-w-3xl mx-auto px-4 py-4">
            <div className="relative flex items-end rounded-3xl border border-border-light bg-surface-chat shadow-md focus-within:shadow-lg transition-shadow">
              <textarea
                ref={textareaRef}
                className="flex-1 resize-none bg-transparent border-0 outline-none text-sm py-3 px-5 max-h-[200px] placeholder:text-muted-foreground/50"
              ...
              {autocompleteOpen && (
                <div
                  ref={autocompleteRef}
                  className="absolute z-[100] bg-popover border border-border-light rounded-xl shadow-lg max-h-60 overflow-y-auto w-80"
                  style={{
                    bottom: '100%',
                    left: '0px',
                    marginBottom: '5px',
              ...
              {/* Send/Stop button: circular button */}
              <div className="p-2">
                {isRunActive ? (
                  <button
                  ...
                  >
                    {interrupting ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Square className="w-3.5 h-3.5 fill-current" />}
                  </button>
                ) : (
                  <button
                    className="w-8 h-8 rounded-full bg-surface-submit text-white flex items-center justify-center hover:bg-surface-submit-hover transition-colors disabled:opacity-40"
                    onClick={handleSendChat}
                    disabled={!chatInput.trim() || sendingChat}
                  >
                    {sendingChat ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowUp className="w-4 h-4" />}
                  </button>
                )}
              </div>
            </div>

            {/* Toolbar below input: settings gear, agents, commands */}
